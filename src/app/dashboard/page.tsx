...
export default function DashboardPage() {
		...
		setActions(
			<Space>
				...
				<Button
					...
					onClick={() =>
						clubs.length > 0 &&
						router.push(`/dashboard/clubs/${clubs[0].id}/sessions/new`)
					}
				>
					New session
				</Button>
	...
	async function handleCreateClub(values: { name: string; location?: string }) {
		try {
			...
			message.error(
				error instanceof Error ? error.message : "Could not create club",
			);
		}
	}
	...
	async function handleJoinClub(values: { joinCode: string }) {
		try {
			await joinClub({
				variables: { joinCode: values.joinCode.trim().toUpperCase() },
			});
			...
			message.error(
				error instanceof Error ? error.message : "Invalid join code",
			);
		}
	}
		...
		<div>
			...
			<div style={{ marginBottom: 16 }}>
				...
				>
					...
					<div
						style={{
							flex: 1,
							height: "1px",
							background: "rgba(138,39,72,0.24)",
						}}
					/>
					{clubs.length > 0 && (
						<Text
				...
				<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
					{clubs.map((club: any) => (
						...
						>
							...
							>
								...
								<div style={{ flex: 1 }}>
									...
									<Text type="secondary" style={{ fontSize: 12 }}>
										{club.location || "No location set"} ·{" "}
										{club.sessions?.length ?? 0} sessions
									</Text>
								</div>
							</div>
							...
							{(() => {
								const draftSessions = (club.sessions ?? []).filter(
									(s: any) => s.status === "DRAFT",
								);
								return draftSessions.length === 0 ? (
									<div style={{ padding: "12px 0" }}>
										<Text type="secondary" style={{ fontSize: 12 }}>
											No sessions yet. Add courts and a roster, then schedule
											the first open play.
										</Text>
									</div>
								) : (
										...
										{draftSessions.map((session: any, idx: number) => (
											<div
												...
												style={{
													...
													borderBottom:
														idx < draftSessions.length - 1
															? "1px solid rgba(138,39,72,0.14)"
															: "none",
												}}
											>
												<div>
													...
													<Text type="secondary" style={{ fontSize: 12 }}>
														{session.sessionDate
															...
															: "No date set"}{" "}
														· {session.courts || "0"} courts ·{" "}
														{session.queueMode || "queue mode not set"}
													</Text>
												</div>
												<Tag
				...
				{clubs.length === 0 && (
					...
					>
						...
						>
							No sessions yet. Add courts and a roster, then schedule the first
							open play.
						</Text>
			...
			>
				...
				>
					<Form.Item
						label="Club name"
						name="name"
						rules={[{ required: true, message: "Enter a club name" }]}
					>
						<Input size="large" />
					</Form.Item>
			...
			>
				<p style={{ color: "#6b7280", marginBottom: 16 }}>
					Ask the club owner for their 6-character join code, then enter it
					below.
				</p>
				<Form
					form={joinForm}
					layout="vertical"
					onFinish={handleJoinClub}
					requiredMark={false}
				>
					<Form.Item
						label="Join code"
